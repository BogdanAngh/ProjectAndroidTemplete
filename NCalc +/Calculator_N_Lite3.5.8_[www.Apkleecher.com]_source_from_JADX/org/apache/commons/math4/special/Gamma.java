...
public class Gamma {
    ...
    class 1 extends ContinuedFraction {
        ...
        protected double getA(int n, double x) {
            return (((2.0d * ((double) n)) + 1.0d) - this.val$a) + x;
        }

        protected double getB(int n, double x) {
            return ((double) n) * (this.val$a - ((double) n));
        }
    }
    ...
    static {
        ...
        HALF_LOG_2_PI = 0.5d * FastMath.log(Angle2D.M_2PI);
    }
    ...
    public static double logGamma(double x) {
        if (Double.isNaN(x) || x <= HALF_LOG_2_PI) {
            return Double.NaN;
        }
        if (x < 0.5d) {
    ...
    public static double regularizedGammaP(double a, double x, double epsilon, int maxIterations) {
        if (Double.isNaN(a) || Double.isNaN(x) || a <= HALF_LOG_2_PI || x < HALF_LOG_2_PI) {
            return Double.NaN;
        }
        if (x == HALF_LOG_2_PI) {
    ...
    public static double regularizedGammaQ(double a, double x, double epsilon, int maxIterations) {
        ...
        return (1.0d / new 1(a).evaluate(x, epsilon, maxIterations)) * FastMath.exp(((-x) + (FastMath.log(x) * a)) - logGamma(a));
    }

    public static double digamma(double x) {
        if (x > HALF_LOG_2_PI && x <= S_LIMIT) {
    ...
    public static double gamma(double x) {
        if (x == FastMath.rint(x) && x <= HALF_LOG_2_PI) {
            return Double.NaN;
        }
        double absX = FastMath.abs(x);
        if (absX > 20.0d) {
            double y = (LANCZOS_G + absX) + 0.5d;
            double gammaAbs = (((SQRT_TWO_PI / x) * FastMath.pow(y, 0.5d + absX)) * FastMath.exp(-y)) * lanczos(absX);
            if (x > HALF_LOG_2_PI) {
                return gammaAbs;
            }
            return -3.141592653589793d / ((FastMath.sin(FastMath.PI * x) * x) * gammaAbs);
        } else if (x >= 1.0d) {
            prod = 1.0d;
            t = x;
            while (t > 2.5d) {
            ...
            return 1.0d / ((1.0d + invGamma1pm1(t)) * prod);
        }
    }
}
