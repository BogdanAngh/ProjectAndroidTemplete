...
public class Erf {
    ...
    public static double erf(double x) {
        if (FastMath.abs(x) > 40.0d) {
            return (double) (x > 0.0d ? 1 : -1);
        }
        double ret = Gamma.regularizedGammaP(0.5d, x * x, CholeskyDecomposition.DEFAULT_RELATIVE_SYMMETRY_THRESHOLD, Dfp.RADIX);
        return x < 0.0d ? -ret : ret;
    }

    public static double erfc(double x) {
        if (FastMath.abs(x) > 40.0d) {
            return (double) (x > 0.0d ? 0 : 2);
        }
        double ret = Gamma.regularizedGammaQ(0.5d, x * x, CholeskyDecomposition.DEFAULT_RELATIVE_SYMMETRY_THRESHOLD, Dfp.RADIX);
