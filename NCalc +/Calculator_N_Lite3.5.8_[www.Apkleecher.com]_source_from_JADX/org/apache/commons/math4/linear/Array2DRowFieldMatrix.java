...
public class Array2DRowFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
    ...
    public Array2DRowFieldMatrix(T[][] d) throws DimensionMismatchException, NullArgumentException, NoDataException {
        this(AbstractFieldMatrix.extractField((FieldElement[][]) d), (FieldElement[][]) d);
    }
    ...
    public Array2DRowFieldMatrix(T[][] d, boolean copyArray) throws DimensionMismatchException, NoDataException, NullArgumentException {
        this(AbstractFieldMatrix.extractField((FieldElement[][]) d), (FieldElement[][]) d, copyArray);
    }
    ...
    public Array2DRowFieldMatrix(T[] v) throws NoDataException {
        this(AbstractFieldMatrix.extractField((FieldElement[]) v), (FieldElement[]) v);
    }
    ...
    public Array2DRowFieldMatrix<T> add(Array2DRowFieldMatrix<T> m) throws MatrixDimensionMismatchException {
        ...
        FieldElement[][] outData = (FieldElement[][]) MathArrays.buildArray(getField(), rowCount, columnCount);
    ...
    public Array2DRowFieldMatrix<T> multiply(Array2DRowFieldMatrix<T> m) throws DimensionMismatchException {
        ...
        FieldElement[][] outData = (FieldElement[][]) MathArrays.buildArray(getField(), nRows, nCols);
        for (int row = 0; row < nRows; row++) {
            FieldElement[] dataRow = this.data[row];
            FieldElement[] outDataRow = outData[row];
            for (int col = 0; col < nCols; col++) {
                ...
                for (int i = 0; i < nSum; i++) {
                    FieldElement sum2 = (FieldElement) sum.add((FieldElement) dataRow[i].multiply(m.data[i][col]));
                }
                outDataRow[col] = sum;
            }
        }
    ...
    public void multiplyEntry(int row, int column, T factor) throws OutOfRangeException {
        checkRowIndex(row);
        checkColumnIndex(column);
        this.data[row][column] = (FieldElement) this.data[row][column].multiply((Object) factor);
    }
    ...
    public T[] operate(T[] v) throws DimensionMismatchException {
        ...
        FieldElement[] out = (FieldElement[]) MathArrays.buildArray(getField(), nRows);
        for (int row = 0; row < nRows; row++) {
            FieldElement[] dataRow = this.data[row];
            T sum = (FieldElement) getField().getZero();
            for (int i = 0; i < nCols; i++) {
                FieldElement sum2 = (FieldElement) sum.add((FieldElement) dataRow[i].multiply(v[i]));
            }
            out[row] = sum;
        }
        return out;
    ...
    public T[] preMultiply(T[] v) throws DimensionMismatchException {
        ...
        FieldElement[] out = (FieldElement[]) MathArrays.buildArray(getField(), nCols);
        for (int col = 0; col < nCols; col++) {
            T sum = (FieldElement) getField().getZero();
            for (int i = 0; i < nRows; i++) {
                FieldElement sum2 = (FieldElement) sum.add((FieldElement) this.data[i][col].multiply(v[i]));
            }
            out[col] = sum;
        }
        return out;
    ...
    private T[][] copyOut() {
        int nRows = getRowDimension();
        FieldElement[][] out = (FieldElement[][]) MathArrays.buildArray(getField(), nRows, getColumnDimension());
        for (int i = 0; i < nRows; i++) {
