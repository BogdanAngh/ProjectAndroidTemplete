...
import org.apache.commons.math4.util.OpenIntToFieldHashMap.Iterator;
...
public class SparseFieldVector<T extends FieldElement<T>> implements FieldVector<T>, Serializable {
    ...
    public T dotProduct(FieldVector<T> v) throws DimensionMismatchException {
        ...
        while (iter.hasNext()) {
            iter.advance();
            FieldElement res2 = (FieldElement) res.add((FieldElement) v.getEntry(iter.key()).multiply(iter.value()));
        }
        return res;
    }
    ...
    public FieldVector<T> mapMultiplyToSelf(T d) throws NullArgumentException {
        ...
        while (iter.hasNext()) {
            iter.advance();
            this.entries.put(iter.key(), (FieldElement) iter.value().multiply((Object) d));
        }
        return this;
    }
    ...
    public FieldVector<T> subtract(FieldVector<T> v) throws DimensionMismatchException {
        ...
        FieldVector<T> res = new SparseFieldVector(this);
        for (int i = 0; i < n; i++) {
    ...
    public FieldVector<T> add(FieldVector<T> v) throws DimensionMismatchException {
        ...
        int n = v.getDimension();
        checkVectorDimensions(n);
        FieldVector<T> res = new SparseFieldVector(this.field, getDimension());
        for (int i = 0; i < n; i++) {
