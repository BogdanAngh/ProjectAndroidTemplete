...
public class MatrixUtils {
    ...
    static {
        ...
        OCTAVE_FORMAT = new RealMatrixFormat("[", "]", BuildConfig.FLAVOR, BuildConfig.FLAVOR, VectorFormat.DEFAULT_SEPARATOR, ", ");
    }
    ...
    public static RealMatrix createRealMatrix(int rows, int columns) {
        return rows * columns <= StaticSettings.MAX_FILE_BUFFER_SIZE ? new Array2DRowRealMatrix(rows, columns) : new BlockRealMatrix(rows, columns);
    }

    public static <T extends FieldElement<T>> FieldMatrix<T> createFieldMatrix(Field<T> field, int rows, int columns) {
        return rows * columns <= StaticSettings.MAX_FILE_BUFFER_SIZE ? new Array2DRowFieldMatrix((Field) field, rows, columns) : new BlockFieldMatrix(field, rows, columns);
    }
    ...
    public static RealMatrix createRealMatrix(double[][] data) throws NullArgumentException, DimensionMismatchException, NoDataException {
        if (data != null && data[0] != null) {
            return data.length * data[0].length <= StaticSettings.MAX_FILE_BUFFER_SIZE ? new Array2DRowRealMatrix(data) : new BlockRealMatrix(data);
        } else {
    ...
    public static <T extends FieldElement<T>> FieldMatrix<T> createFieldMatrix(T[][] data) throws DimensionMismatchException, NoDataException, NullArgumentException {
        if (data != null && data[0] != null) {
            return data.length * data[0].length <= StaticSettings.MAX_FILE_BUFFER_SIZE ? new Array2DRowFieldMatrix((FieldElement[][]) data) : new BlockFieldMatrix(data);
        } else {
            throw new NullArgumentException();
        }
    }
    ...
    public static <T extends FieldElement<T>> FieldMatrix<T> createFieldIdentityMatrix(Field<T> field, int dimension) {
        ...
        for (int row = 0; row < dimension; row++) {
            FieldElement[] dRow = d[row];
            Arrays.fill(dRow, zero);
            dRow[row] = one;
        }
        return new Array2DRowFieldMatrix((Field) field, d, false);
    }
    ...
    public static void checkRowIndex(AnyMatrix m, int row) throws OutOfRangeException {
        if (row < 0 || row >= m.getRowDimension()) {
            throw new OutOfRangeException(LocalizedFormats.ROW_INDEX, Integer.valueOf(row), Integer.valueOf(0), Integer.valueOf(m.getRowDimension() - 1));
        }
    }

    public static void checkColumnIndex(AnyMatrix m, int column) throws OutOfRangeException {
        if (column < 0 || column >= m.getColumnDimension()) {
            throw new OutOfRangeException(LocalizedFormats.COLUMN_INDEX, Integer.valueOf(column), Integer.valueOf(0), Integer.valueOf(m.getColumnDimension() - 1));
        }
    }
    ...
    public static void checkSubMatrixIndex(AnyMatrix m, int[] selectedRows, int[] selectedColumns) throws NoDataException, NullArgumentException, OutOfRangeException {
        ...
        if (selectedRows == null) {
            ...
            while (i < length) {
                checkColumnIndex(m, selectedColumns[i]);
                i++;
            }
        }
    }
    ...
    public static Array2DRowRealMatrix fractionMatrixToRealMatrix(FieldMatrix<Fraction> m) {
        FieldMatrixPreservingVisitor converter = new FractionMatrixConverter();
        m.walkInOptimizedOrder(converter);
        return converter.getConvertedMatrix();
    }

    public static Array2DRowRealMatrix bigFractionMatrixToRealMatrix(FieldMatrix<BigFraction> m) {
        FieldMatrixPreservingVisitor converter = new BigFractionMatrixConverter();
        m.walkInOptimizedOrder(converter);
        return converter.getConvertedMatrix();
    }
    ...
    public static void deserializeRealMatrix(Object instance, String fieldName, ObjectInputStream ois) throws ClassNotFoundException, IOException {
        ...
        try {
            int n = ois.readInt();
            int m = ois.readInt();
            double[][] data = (double[][]) Array.newInstance(Double.TYPE, new int[]{n, m});
            for (int i = 0; i < n; i++) {
                double[] dataI = data[i];
                for (int j = 0; j < m; j++) {
            ...
            if (dSolver.isNonSingular()) {
                ...
                if (tmp1Solver.isNonSingular()) {
                    ...
                    if (tmp2Solver.isNonSingular()) {
                        ...
                        RealMatrix array2DRowRealMatrix = new Array2DRowRealMatrix(n, n);
                        array2DRowRealMatrix.setSubMatrix(result00.getData(), 0, 0);
                        array2DRowRealMatrix.setSubMatrix(result01.getData(), 0, splitIndex1);
                        array2DRowRealMatrix.setSubMatrix(result10.getData(), splitIndex1, 0);
                        array2DRowRealMatrix.setSubMatrix(result11.getData(), splitIndex1, splitIndex1);
