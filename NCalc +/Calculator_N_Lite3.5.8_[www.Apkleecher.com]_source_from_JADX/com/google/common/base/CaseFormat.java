...
public enum CaseFormat {
    LOWER_HYPHEN(CharMatcher.is(Constants.MINUS_UNICODE), "-") {
        ...
        String convert(CaseFormat format, String s) {
            if (format == LOWER_UNDERSCORE) {
                return s.replace(Constants.MINUS_UNICODE, '_');
            }
            if (format == UPPER_UNDERSCORE) {
                return Ascii.toUpperCase(s.replace(Constants.MINUS_UNICODE, '_'));
            }
    ...
    LOWER_UNDERSCORE(CharMatcher.is('_'), "_") {
        ...
        String convert(CaseFormat format, String s) {
            if (format == LOWER_HYPHEN) {
                return s.replace('_', Constants.MINUS_UNICODE);
            }
            if (format == UPPER_UNDERSCORE) {
    ...
    LOWER_CAMEL(CharMatcher.inRange('A', 'Z'), BuildConfig.FLAVOR) {
        String normalizeWord(String word) {
            return CaseFormat.firstCharOnlyToUpper(word);
        }
    },
    UPPER_CAMEL(CharMatcher.inRange('A', 'Z'), BuildConfig.FLAVOR) {
        String normalizeWord(String word) {
            return CaseFormat.firstCharOnlyToUpper(word);
        }
    },
    UPPER_UNDERSCORE(CharMatcher.is('_'), "_") {
        ...
        String convert(CaseFormat format, String s) {
            if (format == LOWER_HYPHEN) {
                return Ascii.toLowerCase(s.replace('_', Constants.MINUS_UNICODE));
            }
            if (format == LOWER_UNDERSCORE) {
