...
class MapMakerInternalMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    static class Segment<K, V> extends ReentrantLock {
        ...
        void initTable(AtomicReferenceArray<ReferenceEntry<K, V>> newTable) {
            this.threshold = (newTable.length() * MapMakerInternalMap.CONTAINS_VALUE_RETRIES) / 4;
        ...
        void drainKeyReferenceQueue() {
            ...
            } while (i != MapMakerInternalMap.DRAIN_MAX);
        }
        ...
        void expand() {
            ...
            if (oldCapacity < MapMakerInternalMap.MAXIMUM_CAPACITY) {
    ...
    static class 1 implements ValueReference<Object, Object> {
        1() {
        }
    ...
    static class 2 extends AbstractQueue<Object> {
        2() {
        }
    ...
    enum EntryFactory {
        ...
        static {
            EntryFactory[][] entryFactoryArr = new EntryFactory[MapMakerInternalMap.CONTAINS_VALUE_RETRIES][];
            entryFactoryArr[0] = new EntryFactory[]{STRONG, STRONG_EXPIRABLE, STRONG_EVICTABLE, STRONG_EXPIRABLE_EVICTABLE};
            entryFactoryArr[EXPIRABLE_MASK] = new EntryFactory[0];
            entryFactoryArr[EVICTABLE_MASK] = new EntryFactory[]{WEAK, WEAK_EXPIRABLE, WEAK_EVICTABLE, WEAK_EXPIRABLE_EVICTABLE};
            factories = entryFactoryArr;
        }
    ...
    static final class EvictionQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
        ...
        class 1 extends AbstractReferenceEntry<K, V> {
        ...
        class 2 extends AbstractSequentialIterator<ReferenceEntry<K, V>> {
            2(ReferenceEntry x0) {
                super(x0);
            }
    ...
    static int rehash(int h) {
        h += (h << 15) ^ -12931;
        h ^= h >>> 10;
        h += h << CONTAINS_VALUE_RETRIES;
        h ^= h >>> 6;
        h += (h << 2) + (h << 14);
        return (h >>> DRAIN_MAX) ^ h;
    }
