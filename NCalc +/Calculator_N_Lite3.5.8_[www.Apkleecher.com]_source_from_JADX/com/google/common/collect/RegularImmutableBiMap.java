...
class RegularImmutableBiMap<K, V> extends ImmutableBiMap<K, V> {
    ...
    class 1 extends ImmutableMapEntrySet<K, V> {
        1() {
        }
    ...
    private final class Inverse extends ImmutableBiMap<V, K> {
        ...
        final class InverseEntrySet extends ImmutableMapEntrySet<V, K> {
            ...
            class 1 extends ImmutableAsList<Entry<V, K>> {
                1() {
                }
    ...
    RegularImmutableBiMap(int n, TerminalEntry<?, ?>[] entriesToAdd) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (ImmutableMapEntry<K, V> keyEntry = nextInKeyBucket; keyEntry != null; keyEntry = keyEntry.getNextInKeyBucket()) {
                ImmutableMap.checkNoConflict(!key.equals(keyEntry.getKey()), ObjectTable.KEY, entry, keyEntry);
            }
            ...
            for (ImmutableMapEntry<K, V> valueEntry = nextInValueBucket; valueEntry != null; valueEntry = valueEntry.getNextInValueBucket()) {
                ImmutableMap.checkNoConflict(!value.equals(valueEntry.getValue()), ObjectTable.VALUE, entry, valueEntry);
            }
