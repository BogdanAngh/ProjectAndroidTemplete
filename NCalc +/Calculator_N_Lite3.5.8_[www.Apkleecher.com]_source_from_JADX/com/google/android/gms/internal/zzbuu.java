...
public final class zzbuu {
    ...
    private static void zza(byte[] bArr, StringBuffer stringBuffer) {
        ...
        stringBuffer.append(Letters.QUOTE);
        for (byte b : bArr) {
            int i = b & MotionEventCompat.ACTION_MASK;
            if (i == 92 || i == 34) {
                stringBuffer.append(Letters.BACKSLASH).append((char) i);
    ...
    private static String zzcE(String str) {
        if (!str.startsWith("http") && str.length() > AnimationUtil.DEFAULT_SHRINK_GROW_DURATION) {
            str = String.valueOf(str.substring(0, AnimationUtil.DEFAULT_SHRINK_GROW_DURATION)).concat("[...]");
        }
        return zzdC(str);
    }
    ...
    private static String zzdC(String str) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (charAt < Letters.SPACE || charAt > '~' || charAt == Letters.QUOTE || charAt == '\'') {
