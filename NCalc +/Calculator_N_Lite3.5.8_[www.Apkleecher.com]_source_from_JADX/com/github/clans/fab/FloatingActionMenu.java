...
public class FloatingActionMenu extends ViewGroup {
    ...
    class 4 implements Runnable {
        ...
        4(FloatingActionButton floatingActionButton, boolean z) {
    ...
    class 6 implements Runnable {
        ...
        6(FloatingActionButton floatingActionButton, boolean z) {
            this.val$fab = floatingActionButton;
            this.val$animate = z;
        }
    ...
    private void initBackgroundDimAnimation() {
        ...
        this.mShowBackgroundAnimator.addUpdateListener(new 1(red, green, blue));
        ...
        this.mHideBackgroundAnimator.addUpdateListener(new 2(red, green, blue));
    }
            ...
            if (getChildAt(i) != this.mImageToggle) {
                ...
                if (fab.getTag(R.id.fab_label) == null) {
                    ...
                    if (fab == this.mMenuButton) {
                        this.mMenuButton.setOnClickListener(new 3());
                    }
                }
            }
        }
        ...
        if (!isOpened()) {
            ...
            for (int i = getChildCount() - 1; i >= 0; i--) {
                ...
                if ((child instanceof FloatingActionButton) && child.getVisibility() != 8) {
                    counter += OPEN_DOWN;
                    this.mUiHandler.postDelayed(new 4((FloatingActionButton) child, animate), (long) delay);
                    delay += this.mAnimationDelayPerItem;
                }
            }
            this.mUiHandler.postDelayed(new 5(), (long) (this.mAnimationDelayPerItem * (counter + OPEN_DOWN)));
        }
    }
    ...
    public void close(boolean animate) {
        if (isOpened()) {
            ...
            for (int i = LABELS_POSITION_LEFT; i < getChildCount(); i += OPEN_DOWN) {
                ...
                if ((child instanceof FloatingActionButton) && child.getVisibility() != 8) {
                    counter += OPEN_DOWN;
                    this.mUiHandler.postDelayed(new 6((FloatingActionButton) child, animate), (long) delay);
                    delay += this.mAnimationDelayPerItem;
                }
            }
            this.mUiHandler.postDelayed(new 7(), (long) (this.mAnimationDelayPerItem * (counter + OPEN_DOWN)));
        }
    }
    ...
    public void hideMenu(boolean animate) {
        if (!isMenuHidden() && !this.mIsMenuButtonAnimationRunning) {
            ...
            if (isOpened()) {
                close(animate);
                this.mUiHandler.postDelayed(new 8(animate), (long) (this.mAnimationDelayPerItem * this.mButtonsCount));
                return;
            }
            if (animate) {
    ...
    public void hideMenuButton(boolean animate) {
        if (!isMenuButtonHidden() && !this.mIsMenuButtonAnimationRunning) {
            ...
            if (isOpened()) {
                close(animate);
                this.mUiHandler.postDelayed(new 9(animate), (long) (this.mAnimationDelayPerItem * this.mButtonsCount));
                return;
            }
