...
public final class GooglePlayServicesUtil {
    ...
    public static int isGooglePlayServicesAvailable(Context context) {
        ...
        try {
            context.getResources().getString(C0074R.string.common_google_play_services_unknown_issue);
        } catch (Throwable th) {
    ...
    private static void zza(int i, Context context, String str) {
        ...
        if (zzi == null) {
            zzi = resources.getString(C0074R.string.common_google_play_services_notification_ticker);
        }
        ...
        if (zzkz.zzai(context)) {
            zzu.zzU(zzlk.zzoV());
            build = new Notification.Builder(context).setSmallIcon(C0074R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(zzi + " " + zzc)).addAction(C0074R.drawable.common_full_open_on_phone, resources.getString(C0074R.string.common_open_on_phone), errorPendingIntent).build();
        } else {
            CharSequence string = resources.getString(C0074R.string.common_google_play_services_notification_ticker);
            if (zzlk.zzoR()) {
                Notification build2;
    ...
    public static Intent zzaT(int i) {
        switch (i) {
            case CompletionEvent.STATUS_FAILURE /*1*/:
            case CompletionEvent.STATUS_CONFLICT /*2*/:
                return zzm.zzcg(GOOGLE_PLAY_SERVICES_PACKAGE);
            case CompletionEvent.STATUS_CANCELED /*3*/:
                return zzm.zzce(GOOGLE_PLAY_SERVICES_PACKAGE);
    ...
    public static boolean zzb(PackageManager packageManager) {
        synchronized (zzoW) {
            if (zzVU == -1) {
                try {
                    if (zzd.zzmn().zza(packageManager.getPackageInfo(GOOGLE_PLAY_SERVICES_PACKAGE, 64), zzc.zzVK[1]) != null) {
                        zzVU = 1;
                    } else {
                        zzVU = GOOGLE_PLAY_SERVICES_VERSION_CODE;
                    }
                } catch (NameNotFoundException e) {
                    zzVU = GOOGLE_PLAY_SERVICES_VERSION_CODE;
                }
