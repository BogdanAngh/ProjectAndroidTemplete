...
public abstract class zzak implements zzaj {
    ...
    String zza(byte[] bArr, String str) throws NoSuchAlgorithmException, UnsupportedEncodingException, IOException {
        ...
        if (bArr.length < 239) {
            ...
            bArr2 = ByteBuffer.allocate(TweenCallback.ANY_BACKWARD).put((byte) bArr.length).put(bArr).put(bArr2).array();
        } else {
            bArr2 = ByteBuffer.allocate(TweenCallback.ANY_BACKWARD).put((byte) bArr.length).put(bArr).array();
        }
        MessageDigest instance = MessageDigest.getInstance("MD5");
        instance.update(bArr2);
        bArr2 = ByteBuffer.allocate(GL20.GL_DEPTH_BUFFER_BIT).put(instance.digest()).put(bArr2).array();
        byte[] bArr3 = new byte[GL20.GL_DEPTH_BUFFER_BIT];
