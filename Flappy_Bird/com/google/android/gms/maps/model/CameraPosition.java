...
public final class CameraPosition implements SafeParcelable {
    ...
    public static CameraPosition createFromAttributes(Context context, AttributeSet attrs) {
        ...
        LatLng latLng = new LatLng((double) (obtainAttributes.hasValue(C0074R.styleable.MapAttrs_cameraTargetLat) ? obtainAttributes.getFloat(C0074R.styleable.MapAttrs_cameraTargetLat, 0.0f) : 0.0f), (double) (obtainAttributes.hasValue(C0074R.styleable.MapAttrs_cameraTargetLng) ? obtainAttributes.getFloat(C0074R.styleable.MapAttrs_cameraTargetLng, 0.0f) : 0.0f));
        Builder builder = builder();
        builder.target(latLng);
        if (obtainAttributes.hasValue(C0074R.styleable.MapAttrs_cameraZoom)) {
            builder.zoom(obtainAttributes.getFloat(C0074R.styleable.MapAttrs_cameraZoom, 0.0f));
        }
        if (obtainAttributes.hasValue(C0074R.styleable.MapAttrs_cameraBearing)) {
            builder.bearing(obtainAttributes.getFloat(C0074R.styleable.MapAttrs_cameraBearing, 0.0f));
        }
        if (obtainAttributes.hasValue(C0074R.styleable.MapAttrs_cameraTilt)) {
            builder.tilt(obtainAttributes.getFloat(C0074R.styleable.MapAttrs_cameraTilt, 0.0f));
        }
