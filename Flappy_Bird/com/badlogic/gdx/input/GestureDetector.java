...
public class GestureDetector extends InputAdapter {
    ...
    class C03651 extends Task {
        C03651() {
        }

        public void run() {
            if (!GestureDetector.this.longPressFired) {
                GestureDetector.this.longPressFired = GestureDetector.this.listener.longPress(GestureDetector.this.pointer1.f100x, GestureDetector.this.pointer1.f101y);
            }
        }
    }
    ...
    public boolean touchUp(float x, float y, int pointer, int button) {
        ...
        if (this.inTapSquare) {
            ...
            this.panning = true;
            if (pointer == 0) {
                this.tracker.start(this.pointer2.f100x, this.pointer2.f101y, Gdx.input.getCurrentEventTime());
            } else {
                this.tracker.start(this.pointer1.f100x, this.pointer1.f101y, Gdx.input.getCurrentEventTime());
            }
            return false;
        } else {
