...
class ComparableTimSort {
    ...
    private void mergeAt(int i) {
        ...
        this.stackSize--;
        int k = gallopRight((Comparable) this.f86a[base2], this.f86a, base1, len1, 0);
        base1 += k;
        len1 -= k;
        if (len1 != 0) {
            len2 = gallopLeft((Comparable) this.f86a[(base1 + len1) - 1], this.f86a, base2, len2, len2 - 1);
            if (len2 == 0) {
                return;
            }
            if (len1 <= len2) {
    ...
    private Object[] ensureCapacity(int minCapacity) {
        if (this.tmp.length < minCapacity) {
            ...
            if (newSize < 0) {
                ...
                newSize = Math.min(newSize, this.f86a.length >>> 1);
            }
