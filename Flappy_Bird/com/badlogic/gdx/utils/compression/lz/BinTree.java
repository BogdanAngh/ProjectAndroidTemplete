...
public class BinTree extends InWindow {
    ...
    public boolean Create(int historySize, int keepAddBufferBefore, int matchMaxLen, int keepAddBufferAfter) {
        ...
        super.Create(historySize + keepAddBufferBefore, matchMaxLen + keepAddBufferAfter, ((((historySize + keepAddBufferBefore) + matchMaxLen) + keepAddBufferAfter) / 2) + GL20.GL_DEPTH_BUFFER_BIT);
        ...
        if (this.HASH_ARRAY) {
            ...
            hs |= hs >> 2;
            hs |= hs >> 4;
            hs = ((hs | (hs >> 8)) >> kStartMaxLen) | GameRequest.TYPE_ALL;
            if (hs > 16777216) {
                hs >>= kStartMaxLen;
            }
            this._hashMask = hs;
    ...
    public int GetMatches(int[] distances) throws IOException {
        ...
        if (this.HASH_ARRAY) {
            i = this._bufferBase[cur] & Keys.F12;
            int temp = CrcTable[r0] ^ (this._bufferBase[cur + kStartMaxLen] & Keys.F12);
            hash2Value = temp & Place.TYPE_SUBLOCALITY_LEVEL_1;
            temp ^= (this._bufferBase[cur + 2] & Keys.F12) << 8;
            hash3Value = temp & GameRequest.TYPE_ALL;
            i = this._bufferBase[cur + 3] & Keys.F12;
    ...
    public void Skip(int num) throws IOException {
        do {
            ...
            if (this.HASH_ARRAY) {
                i = this._bufferBase[cur] & Keys.F12;
                int temp = CrcTable[r0] ^ (this._bufferBase[cur + kStartMaxLen] & Keys.F12);
                int hash2Value = temp & Place.TYPE_SUBLOCALITY_LEVEL_1;
                this._hash[hash2Value] = this._pos;
                temp ^= (this._bufferBase[cur + 2] & Keys.F12) << 8;
                int hash3Value = temp & GameRequest.TYPE_ALL;
                this._hash[hash3Value + kHash3Offset] = this._pos;
                i = this._bufferBase[cur + 3] & Keys.F12;
                hashValue = ((CrcTable[r0] << 5) ^ temp) & this._hashMask;
            } else {
    ...
    static {
        ...
        for (int i = kEmptyHashValue; i < GL20.GL_DEPTH_BUFFER_BIT; i += kStartMaxLen) {
            int r = i;
