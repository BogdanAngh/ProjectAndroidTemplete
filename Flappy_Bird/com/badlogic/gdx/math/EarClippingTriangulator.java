...
public class EarClippingTriangulator {
    ...
    private boolean isEarTip(int earTipIndex) {
        ...
        while (i != previousIndex) {
            if (vertexTypes[i] != CONVEX) {
                ...
                if (computeSpannedAreaSign(p1x, p1y, p2x, p2y, vx, vy) >= 0 && computeSpannedAreaSign(p2x, p2y, p3x, p3y, vx, vy) >= 0 && computeSpannedAreaSign(p3x, p3y, p1x, p1y, vx, vy) >= 0) {
                    return false;
                }
            }
            i = nextIndex(i);
    ...
    private static boolean areVerticesClockwise(float[] vertices, int offset, int count) {
        ...
        p1y = vertices[count + CONCAVE];
        if (((p1x * vertices[CONVEX]) + area) - (vertices[0] * p1y) >= 0.0f) {
            z = false;
        }
        return z;
    }
