...
public class AmbientCubemap {
    ...
    private static final float clamp(float v) {
        ...
        return v > TextTrackStyle.DEFAULT_FONT_SCALE ? TextTrackStyle.DEFAULT_FONT_SCALE : v;
    }
    ...
    public Color getColor(Color out, int side) {
        side *= 3;
        return out.set(this.data[side], this.data[side + 1], this.data[side + 2], TextTrackStyle.DEFAULT_FONT_SCALE);
    }
    ...
    public AmbientCubemap add(float r, float g, float b, float x, float y, float z) {
        ...
        float d = (x2 + y2) + z2;
        if (d != 0.0f) {
            d = (TextTrackStyle.DEFAULT_FONT_SCALE / d) * (TextTrackStyle.DEFAULT_FONT_SCALE + d);
            float rd = r * d;
            float gd = g * d;
            float bd = b * d;
            int idx = x > 0.0f ? 0 : 3;
    ...
    public AmbientCubemap add(Color color, Vector3 direction) {
        return add(color.f40r, color.f39g, color.f38b, direction.f105x, direction.f106y, direction.f107z);
    }

    public AmbientCubemap add(float r, float g, float b, Vector3 direction) {
        return add(r, g, b, direction.f105x, direction.f106y, direction.f107z);
    }
    ...
    public AmbientCubemap add(Color color, Vector3 point, Vector3 target) {
        return add(color.f40r, color.f39g, color.f38b, target.f105x - point.f105x, target.f106y - point.f106y, target.f107z - point.f107z);
    }

    public AmbientCubemap add(Color color, Vector3 point, Vector3 target, float intensity) {
        float t = intensity / (TextTrackStyle.DEFAULT_FONT_SCALE + target.dst(point));
        return add(color.f40r * t, color.f39g * t, color.f38b * t, target.f105x - point.f105x, target.f106y - point.f106y, target.f107z - point.f107z);
    }
