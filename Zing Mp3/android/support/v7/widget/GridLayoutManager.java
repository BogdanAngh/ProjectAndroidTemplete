...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                view = this.mSet[i];
                measureChild(view, C0989C.ENCODING_PCM_32BIT, true);
        ...
        for (i = 0; i < count; i++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    wSpec = LayoutManager.getChildMeasureSpec(totalSpaceInOther, C0989C.ENCODING_PCM_32BIT, horizontalInsets, lp.width, DEBUG);
                    hSpec = MeasureSpec.makeMeasureSpec(maxSize - verticalInsets, C0989C.ENCODING_PCM_32BIT);
                } else {
                    wSpec = MeasureSpec.makeMeasureSpec(maxSize - horizontalInsets, C0989C.ENCODING_PCM_32BIT);
                    hSpec = LayoutManager.getChildMeasureSpec(totalSpaceInOther, C0989C.ENCODING_PCM_32BIT, verticalInsets, lp.height, DEBUG);
                }
