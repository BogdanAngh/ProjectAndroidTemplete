...
public class DeviceAuthDialog extends DialogFragment {
    ...
    class C08741 implements Callback {
        C08741() {
        }
    ...
    class C08752 implements ImageRequest.Callback {
        final /* synthetic */ TextView val$textView;

        C08752(TextView textView) {
            this.val$textView = textView;
        }

        public void onCompleted(ImageResponse response) {
            if (response.getBitmap() != null) {
                this.val$textView.setCompoundDrawablesWithIntrinsicBounds(null, null, new BitmapDrawable(DeviceAuthDialog.this.getResources(), Bitmap.createScaledBitmap(response.getBitmap(), 24, 24, false)), null);
            }
        }
    }
    ...
    class C08785 implements Callback {
        C08785() {
        }
    ...
    class C08796 implements DialogInterface.OnClickListener {
        C08796() {
        }
    ...
    class C08807 implements DialogInterface.OnClickListener {
        final /* synthetic */ String val$accessToken;
        final /* synthetic */ PermissionsPair val$permissions;
        final /* synthetic */ String val$userId;

        C08807(String str, PermissionsPair permissionsPair, String str2) {
            this.val$userId = str;
    ...
    /* renamed from: com.facebook.login.DeviceAuthDialog.8 */
    class C08818 implements Callback {
        final /* synthetic */ String val$accessToken;
        ...
        public void onCompleted(GraphResponse response) {
            if (!DeviceAuthDialog.this.completed.get()) {
                ...
                try {
                    JSONObject jsonObject = response.getJSONObject();
                    String userId = jsonObject.getString(TtmlNode.ATTR_ID);
    ...
    private static class RequestState implements Parcelable {
        ...
        static class C08821 implements Creator<RequestState> {
            C08821() {
            }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        this.dialog = new Dialog(getActivity(), C0369R.style.com_facebook_auth_dialog);
    ...
    public void startLogin(Request request) {
        ...
        new GraphRequest(null, DEVICE_LOGIN_ENDPOINT, parameters, HttpMethod.POST, new C08741()).executeAsync();
    }
    ...
    private void appendIconToTextView(TextView textView, String iconUriString) {
        ImageDownloader.downloadAsync(new Builder(getContext(), Uri.parse(iconUriString)).setCallback(new C08752(textView)).build());
    }
    ...
    private View initializeContentView(boolean isSmartLogin) {
        ...
        LayoutInflater inflater = getActivity().getLayoutInflater();
        if (isSmartLogin) {
            view = inflater.inflate(C0369R.layout.com_facebook_smart_device_dialog_fragment, null);
            ...
            if (settings.getSmartLoginBookmarkIconURL() != null) {
                appendIconToTextView((TextView) view.findViewById(C0369R.id.com_facebook_smart_instructions_2), settings.getSmartLoginBookmarkIconURL());
            }
            if (settings.getSmartLoginMenuIconURL() != null) {
                appendIconToTextView((TextView) view.findViewById(C0369R.id.com_facebook_smart_instructions_1), settings.getSmartLoginMenuIconURL());
            }
        } else {
            view = inflater.inflate(C0369R.layout.com_facebook_device_auth_dialog_fragment, null);
        }
        this.progressBar = (ProgressBar) view.findViewById(C0369R.id.progress_bar);
        this.confirmationCode = (TextView) view.findViewById(C0369R.id.confirmation_code);
        ((Button) view.findViewById(C0369R.id.cancel_button)).setOnClickListener(new C08763());
        ((TextView) view.findViewById(C0369R.id.com_facebook_device_auth_instructions)).setText(Html.fromHtml(getString(C0369R.string.com_facebook_device_auth_instructions)));
        return view;
    }
    ...
    private void schedulePoll() {
        this.scheduledPoll = DeviceAuthMethodHandler.getBackgroundExecutor().schedule(new C08774(), this.currentRequestState.getInterval(), TimeUnit.SECONDS);
    }

    private GraphRequest getPollRequest() {
        Bundle parameters = new Bundle();
        parameters.putString("code", this.currentRequestState.getRequestCode());
        return new GraphRequest(null, DEVICE_LOGIN_STATUS_ENDPOINT, parameters, HttpMethod.POST, new C08785());
    }

    private void presentConfirmation(String userId, PermissionsPair permissions, String accessToken, String name) {
        String message = getResources().getString(C0369R.string.com_facebook_smart_login_confirmation_title);
        String continueFormat = getResources().getString(C0369R.string.com_facebook_smart_login_confirmation_continue_as);
        String cancel = getResources().getString(C0369R.string.com_facebook_smart_login_confirmation_cancel);
        String continueText = String.format(continueFormat, new Object[]{name});
        AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
        builder.setMessage(message).setCancelable(true).setNegativeButton(continueText, new C08807(userId, permissions, accessToken)).setPositiveButton(cancel, new C08796());
        builder.create().show();
    }
    ...
    private void onSuccess(String accessToken) {
        ...
        new GraphRequest(temporaryToken, "me", parameters, HttpMethod.GET, new C08818(accessToken)).executeAsync();
    }
