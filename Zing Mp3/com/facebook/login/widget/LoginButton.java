...
public class LoginButton extends FacebookButtonBase {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        class C09041 implements DialogInterface.OnClickListener {
            ...
            C09041(LoginManager loginManager) {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                String message;
                String logout = LoginButton.this.getResources().getString(C0369R.string.com_facebook_loginview_log_out_action);
                String cancel = LoginButton.this.getResources().getString(C0369R.string.com_facebook_loginview_cancel_action);
                Profile profile = Profile.getCurrentProfile();
                if (profile == null || profile.getName() == null) {
                    message = LoginButton.this.getResources().getString(C0369R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    message = String.format(LoginButton.this.getResources().getString(C0369R.string.com_facebook_loginview_logged_in_as), new Object[]{profile.getName()});
                }
                Builder builder = new Builder(context);
                builder.setMessage(message).setCancelable(true).setPositiveButton(logout, new C09041(loginManager)).setNegativeButton(cancel, null);
                builder.create().show();
                return;
            }
            loginManager.logOut();
    ...
    class C09011 implements Runnable {
        ...
        class C09001 implements Runnable {
            ...
            C09001(FetchedAppSettings fetchedAppSettings) {
        ...
        public void run() {
            LoginButton.this.getActivity().runOnUiThread(new C09001(FetchedAppSettingsManager.queryAppSettings(this.val$appId, false)));
        }
    }

    /* renamed from: com.facebook.login.widget.LoginButton.2 */
    class C09022 extends AccessTokenTracker {
        C09022() {
        }
    ...
    private void checkToolTipSettings() {
        switch (C09033.$SwitchMap$com$facebook$login$widget$LoginButton$ToolTipMode[this.toolTipMode.ordinal()]) {
            case C1569R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped /*1*/:
                FacebookSdk.getExecutor().execute(new C09011(Utility.getMetadataApplicationId(getContext())));
            case C1569R.styleable.com_facebook_login_view_com_facebook_logout_text /*2*/:
                displayToolTip(getResources().getString(C0369R.string.com_facebook_tooltip_default));
            default:
        }
    }
    ...
    protected void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C0369R.color.com_facebook_blue));
            this.loginText = "Log in with Facebook";
        } else {
            this.accessTokenTracker = new C09022();
        }
        setButtonText();
    }
    ...
    protected int getDefaultStyleResource() {
        return C0369R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0369R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C0369R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C0369R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C0369R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C0369R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C0369R.string.com_facebook_loginview_log_in_button_long);
            logInWidth = measureButtonWidth(text);
            if (resolveSize(logInWidth, widthMeasureSpec) < logInWidth) {
                text = resources.getString(C0369R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        text = this.logoutText;
        if (text == null) {
            text = resources.getString(C0369R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            ...
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C0369R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C0369R.string.com_facebook_loginview_log_in_button_long);
            int width = getWidth();
