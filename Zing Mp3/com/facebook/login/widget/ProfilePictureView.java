...
public class ProfilePictureView extends FrameLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (MeasureSpec.getMode(heightMeasureSpec) != C0989C.ENCODING_PCM_32BIT && params.height == SMALL) {
            newHeight = getPresetSizeInPixels(IS_CROPPED_DEFAULT_VALUE);
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(newHeight, C0989C.ENCODING_PCM_32BIT);
            customMeasure = IS_CROPPED_DEFAULT_VALUE;
        }
        if (MeasureSpec.getMode(widthMeasureSpec) != C0989C.ENCODING_PCM_32BIT && params.width == SMALL) {
            newWidth = getPresetSizeInPixels(IS_CROPPED_DEFAULT_VALUE);
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(newWidth, C0989C.ENCODING_PCM_32BIT);
            customMeasure = IS_CROPPED_DEFAULT_VALUE;
        }
        if (customMeasure) {
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, C0369R.styleable.com_facebook_profile_picture_view);
        setPresetSize(a.getInt(C0369R.styleable.com_facebook_profile_picture_view_com_facebook_preset_size, CUSTOM));
        this.isCropped = a.getBoolean(C0369R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped, IS_CROPPED_DEFAULT_VALUE);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            setImageBitmap(BitmapFactory.decodeResource(getResources(), isCropped() ? C0369R.drawable.com_facebook_profile_picture_blank_square : C0369R.drawable.com_facebook_profile_picture_blank_portrait));
            return;
        }
    ...
    private void sendImageRequest(boolean allowCachedResponse) {
        ImageRequest request = new Builder(getContext(), ImageRequest.getProfilePictureUri(this.profileId, this.queryWidth, this.queryHeight)).setAllowCachedRedirects(allowCachedResponse).setCallerTag(this).setCallback(new C09051()).build();
        if (this.lastRequest != null) {
            ImageDownloader.cancelRequest(this.lastRequest);
        }
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case LARGE /*-4*/:
                dimensionId = C0369R.dimen.com_facebook_profilepictureview_preset_size_large;
                break;
            case NORMAL /*-3*/:
                dimensionId = C0369R.dimen.com_facebook_profilepictureview_preset_size_normal;
                break;
            case SMALL /*-2*/:
                dimensionId = C0369R.dimen.com_facebook_profilepictureview_preset_size_small;
                break;
            case CUSTOM /*-1*/:
                if (forcePreset) {
                    dimensionId = C0369R.dimen.com_facebook_profilepictureview_preset_size_normal;
