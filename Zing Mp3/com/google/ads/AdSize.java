...
public final class AdSize {
    ...
    public boolean isSizeAppropriate(int i, int i2) {
        int width = getWidth();
        int height = getHeight();
        return ((float) i) <= ((float) width) * 1.25f && ((float) i) >= ((float) width) * DefaultLoadControl.DEFAULT_HIGH_BUFFER_LOAD && ((float) i2) <= ((float) height) * 1.25f && ((float) i2) >= ((float) height) * DefaultLoadControl.DEFAULT_HIGH_BUFFER_LOAD;
    }
