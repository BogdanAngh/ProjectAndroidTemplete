    ...
    public void load(Map<String, String> map) throws MappingException {
        ...
        for (Entry entry : map.entrySet()) {
            ...
            if (field2 != null) {
                try {
                    ...
                    zzb.zzdi(new StringBuilder(String.valueOf(str).length() + 49).append("Server option \"").append(str).append("\" could not be set: Illegal Access").toString());
                } catch (IllegalArgumentException e2) {
                    str = (String) entry.getKey();
                    zzb.zzdi(new StringBuilder(String.valueOf(str).length() + 43).append("Server option \"").append(str).append("\" could not be set: Bad Type").toString());
                }
            } else {
                str = (String) entry.getKey();
                String str2 = (String) entry.getValue();
                zzb.zzdg(new StringBuilder((String.valueOf(str).length() + 31) + String.valueOf(str2).length()).append("Unexpected server option: ").append(str).append(" = \"").append(str2).append("\"").toString());
            }
        }
        ...
        for (Field field22 : hashMap.values()) {
            if (((Parameter) field22.getAnnotation(Parameter.class)).required()) {
                ...
                zzb.zzdi(str2.length() != 0 ? str3.concat(str2) : new String(str3));
