...
public final class FragmentedMp4Extractor implements Extractor {
    ...
    private void onMoovContainerAtomRead(ContainerAtom moov) {
        ...
        for (i = STATE_READING_ATOM_HEADER; i < moovContainerChildrenSize; i += STATE_READING_ATOM_PAYLOAD) {
            ...
            if (atom2.type == Atom.TYPE_trak) {
                track = AtomParsers.parseTrak(atom2, moov.getLeafAtomOfType(Atom.TYPE_mvhd), false);
                if (track != null) {
    ...
    private static void parseTraf(ContainerAtom traf, SparseArray<TrackBundle> trackBundleArray, int flags, byte[] extendedTypeScratch) throws ParserException {
        ...
        if (trackBundle != null) {
            ...
            trackBundle.currentSampleIndex = STATE_READING_ATOM_HEADER;
            fragment.reset();
    ...
    private static void parseTrun(TrackBundle trackBundle, long decodeTime, int flags, ParsableByteArray trun) {
        ...
        boolean sampleDurationsPresent = (atomFlags & AccessibilityNodeInfoCompat.ACTION_NEXT_AT_MOVEMENT_GRANULARITY) != 0;
        boolean sampleSizesPresent = (atomFlags & AdRequest.MAX_CONTENT_URL_LENGTH) != 0;
        boolean sampleFlagsPresent = (atomFlags & AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT) != 0;
        boolean sampleCompositionTimeOffsetsPresent = (atomFlags & ItemAnimator.FLAG_MOVED) != 0;
        long edtsOffset = 0;
    ...
    private static ChunkIndex parseSidx(ParsableByteArray atom, long inputPosition) throws ParserException {
        ...
        long timeUs = Util.scaleLargeTimestamp(time, C0989C.MICROS_PER_SECOND, timescale);
        for (int i = STATE_READING_ATOM_HEADER; i < referenceCount; i += STATE_READING_ATOM_PAYLOAD) {
            ...
            timeUs = Util.scaleLargeTimestamp(time, C0989C.MICROS_PER_SECOND, timescale);
