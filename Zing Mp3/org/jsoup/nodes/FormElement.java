...
public class FormElement extends Element {
    ...
    public Connection submit() {
        String action = hasAttr(NativeProtocol.WEB_DIALOG_ACTION) ? absUrl(NativeProtocol.WEB_DIALOG_ACTION) : baseUri();
    ...
    public List<KeyVal> formData() {
        ...
        while (it.hasNext()) {
            ...
            if (el.tag().isFormSubmittable()) {
                String name = el.attr(ShareConstants.WEB_DIALOG_PARAM_NAME);
                if (name.length() != 0) {
