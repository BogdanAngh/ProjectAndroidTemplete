...
enum HtmlTreeBuilderState {
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            if (t.isComment()) {
                ...
                if (t.isStartTag() && t.asStartTag().name().equals("html")) {
                    ...
                    if (t.isEndTag()) {
                        if (StringUtil.in(t.asEndTag().name(), TtmlNode.TAG_HEAD, TtmlNode.TAG_BODY, "html", TtmlNode.TAG_BR)) {
    ...
    BeforeHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            if (t.isComment()) {
                ...
                if (t.isStartTag() && t.asStartTag().name().equals(TtmlNode.TAG_HEAD)) {
                ...
                if (t.isEndTag()) {
                    if (StringUtil.in(t.asEndTag().name(), TtmlNode.TAG_HEAD, TtmlNode.TAG_BODY, "html", TtmlNode.TAG_BR)) {
                        tb.process(new StartTag(TtmlNode.TAG_HEAD));
                        return tb.process(t);
                    }
                }
                if (t.isEndTag()) {
    ...
    InHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            switch (AnonymousClass24.$SwitchMap$org$jsoup$parser$Token$TokenType[t.type.ordinal()]) {
                ...
                case C1569R.styleable.com_facebook_login_view_com_facebook_tooltip_mode /*3*/:
                    ...
                    if (StringUtil.in(name, "base", "basefont", "bgsound", "command", ShareConstants.WEB_DIALOG_PARAM_LINK)) {
                ...
                case C1569R.styleable.com_facebook_like_view_com_facebook_auxiliary_view_position /*4*/:
                    ...
                    if (StringUtil.in(name, TtmlNode.TAG_BODY, "html", TtmlNode.TAG_BR)) {
                        return anythingElse(t, tb);
                    }
                    tb.error(this);
                    return false;
        ...
        private boolean anythingElse(Token t, TreeBuilder tb) {
            tb.process(new EndTag(TtmlNode.TAG_HEAD));
            return tb.process(t);
        }
    },
    InHeadNoscript {
    ...
    AfterHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (HtmlTreeBuilderState.isWhitespace(t)) {
                ...
                if (name.equals(TtmlNode.TAG_BODY)) {
                    ...
                    if (StringUtil.in(name, "base", "basefont", "bgsound", ShareConstants.WEB_DIALOG_PARAM_LINK, "meta", "noframes", "script", TtmlNode.TAG_STYLE, ShareConstants.WEB_DIALOG_PARAM_TITLE)) {
                        tb.error(this);
                ...
                if (StringUtil.in(t.asEndTag().name(), TtmlNode.TAG_BODY, "html")) {
                    anythingElse(t, tb);
                } else {
                    tb.error(this);
                    return false;
        ...
        private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
            tb.process(new StartTag(TtmlNode.TAG_BODY));
            tb.framesetOk(true);
            return tb.process(t);
        }
    },
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            switch (AnonymousClass24.$SwitchMap$org$jsoup$parser$Token$TokenType[t.type.ordinal()]) {
                    ...
                    if (!name.equals("html")) {
                        if (!StringUtil.in(name, Constants.InBodyStartToHead)) {
                            if (!name.equals(TtmlNode.TAG_BODY)) {
                                            ...
                                            if (!StringUtil.in(name, Constants.InBodyStartPreListing)) {
                                                if (name.equals("form")) {
                                                    if (tb.getFormElement() == null) {
                                                        if (tb.inButtonScope(TtmlNode.TAG_P)) {
                                                            tb.process(new EndTag(TtmlNode.TAG_P));
                                                        }
                                                                            ...
                                                                            if (!StringUtil.in(name, Constants.InBodyStartApplets)) {
                                                                                if (!name.equals("table")) {
                                                                                    if (!StringUtil.in(name, Constants.InBodyStartEmptyFormatters)) {
                                                                                                ...
                                                                                                if (!name.equals("hr")) {
                                                                                                    if (!name.equals("image")) {
                                                                                                        ...
                                                                                                        if (tb.getFormElement() == null) {
                                                                                                            ...
                                                                                                            if (startTag.attributes.hasKey(NativeProtocol.WEB_DIALOG_ACTION)) {
                                                                                                                tb.getFormElement().attr(NativeProtocol.WEB_DIALOG_ACTION, startTag.attributes.get(NativeProtocol.WEB_DIALOG_ACTION));
                                                                                                            }
                                                                                                            ...
                                                                                                            inputAttribs.put(ShareConstants.WEB_DIALOG_PARAM_NAME, "isindex");
                                                                                        ...
                                                                                        tb.reconstructFormattingElements();
                                                                                        if (!tb.insertEmpty(startTag).attr(ShareConstants.MEDIA_TYPE).equalsIgnoreCase("hidden")) {
                                                                                            tb.framesetOk(false);
                                                                                            break;
                                                                                        }
                                                                                    }
                                ...
                                tb.error(this);
                                stack = tb.getStack();
                                if (stack.size() != 1 && (stack.size() <= 2 || ((Element) stack.get(1)).nodeName().equals(TtmlNode.TAG_BODY))) {
                                    if (tb.framesetOk()) {
                ...
                case C1569R.styleable.com_facebook_like_view_com_facebook_auxiliary_view_position /*4*/:
                    ...
                    name = endTag.name();
                    if (name.equals(TtmlNode.TAG_BODY)) {
                        if (tb.inScope(TtmlNode.TAG_BODY)) {
                    ...
                    if (name.equals("html")) {
                        if (tb.process(new EndTag(TtmlNode.TAG_BODY))) {
                    ...
                    if (StringUtil.in(name, Constants.InBodyStartApplets)) {
                        if (!tb.inScope(ShareConstants.WEB_DIALOG_PARAM_NAME)) {
                            if (tb.inScope(name)) {
                                tb.generateImpliedEndTags();
                                if (!tb.currentElement().nodeName().equals(name)) {
                                    tb.error(this);
                    ...
                    tb.error(this);
                    tb.process(new StartTag(TtmlNode.TAG_BR));
                    return false;
                    break;
            ...
            if (t.isCharacter()) {
                ...
                if (name.equals(ShareConstants.FEED_CAPTION_PARAM)) {
                    ...
                    if (StringUtil.in(name, "td", "th", "tr")) {
                        ...
                        if (name.equals("input")) {
                            if (!startTag.attributes.get(ShareConstants.MEDIA_TYPE).equalsIgnoreCase("hidden")) {
                                return anythingElse(t, tb);
                            }
                            tb.insertEmpty(startTag);
                            return true;
                ...
                name = t.asEndTag().name();
                if (!name.equals("table")) {
                    if (!StringUtil.in(name, TtmlNode.TAG_BODY, ShareConstants.FEED_CAPTION_PARAM, "col", "colgroup", "html", "tbody", "td", "tfoot", "th", "thead", "tr")) {
                        return anythingElse(t, tb);
                    }
                    tb.error(this);
                    return false;
            ...
            switch (AnonymousClass24.$SwitchMap$org$jsoup$parser$Token$TokenType[t.type.ordinal()]) {
                ...
                case C1569R.styleable.com_facebook_like_view_com_facebook_auxiliary_view_position /*4*/:
                    ...
                    if (StringUtil.in(name, "tbody", "tfoot", "thead")) {
                        ...
                        if (!StringUtil.in(name, TtmlNode.TAG_BODY, ShareConstants.FEED_CAPTION_PARAM, "col", "colgroup", "html", "td", "th", "tr")) {
                            return anythingElse(t, tb);
                        }
                        tb.error(this);
                        return false;
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            if (t.isStartTag()) {
                ...
                if (name.equals("tr")) {
                    ...
                    if (!StringUtil.in(name, "tbody", "tfoot", "thead")) {
                        if (!StringUtil.in(name, TtmlNode.TAG_BODY, ShareConstants.FEED_CAPTION_PARAM, "col", "colgroup", "html", "td", "th")) {
                            return anythingElse(t, tb);
                        }
                        tb.error(this);
                        return false;
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isEndTag()) {
                ...
                if (!StringUtil.in(name, "td", "th")) {
                    if (StringUtil.in(name, TtmlNode.TAG_BODY, ShareConstants.FEED_CAPTION_PARAM, "col", "colgroup", "html")) {
                        tb.error(this);
                        return false;
                    }
    ...
    private static final class Constants {
        ...
        static {
            InBodyStartToHead = new String[]{"base", "basefont", "bgsound", "command", ShareConstants.WEB_DIALOG_PARAM_LINK, "meta", "noframes", "script", TtmlNode.TAG_STYLE, ShareConstants.WEB_DIALOG_PARAM_TITLE};
            ...
            InBodyStartMedia = new String[]{"param", ShareConstants.FEED_SOURCE_PARAM, "track"};
            InBodyStartInputAttribs = new String[]{ShareConstants.WEB_DIALOG_PARAM_NAME, NativeProtocol.WEB_DIALOG_ACTION, "prompt"};
            InBodyStartOptions = new String[]{"optgroup", "option"};
            InBodyStartRuby = new String[]{"rp", "rt"};
            InBodyStartDrop = new String[]{ShareConstants.FEED_CAPTION_PARAM, "col", "colgroup", "frame", TtmlNode.TAG_HEAD, "tbody", "td", "tfoot", "th", "thead", "tr"};
